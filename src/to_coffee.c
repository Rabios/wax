#ifndef WAX_TO_COFFEE
#define WAX_TO_COFFEE

#include "text.c"
#include "parser.c"
#include "common.c"

str_t zero_to_coffee(type_t* typ){
  str_t out = str_new();
  if (typ->tag == TYP_INT){
    str_add(&out,"0");
...
str_t expr_to_coffee(expr_t* expr, int indent){
  ...
  if (expr->key == EXPR_LET){
    ...
    str_add(&out,"=");
    str_add(&out,zero_to_coffee( (type_t*)(CHILD2->term) ).data);
    
  }else if (expr->key == EXPR_SET){
    str_add(&out, expr_to_coffee(CHILD1,-1).data);
    str_add(&out,"=");
    str_add(&out, expr_to_coffee(CHILD2,-1).data );
    ...
    str_add(&out, expr_to_coffee(CHILD1,-1).data);
    str_add(&out, ")\n");
    str_add(&out, expr_to_coffee(CHILD2,indent).data);
    INDENT2(indent);
    if (CHILD3){
      str_add(&out, "else\n");
      str_add(&out, expr_to_coffee(CHILD3,indent).data);
      INDENT2(indent);
    }
    ...
    str_add(&out, expr_to_coffee(CHILD2,-1).data);
    str_add(&out, ")||(");
    str_add(&out, expr_to_coffee(CHILD3,-1).data);
    str_add(&out, "))");
	...
	str_add(&out, expr_to_coffee(CHILD3,-1).data);
	str_add(&out, "\n");
	str_add(&out, expr_to_coffee(CHILDN,indent).data);
	INDENT2(indent+1);
	str_add(&out, expr_to_coffee(CHILD1,-1).data);
    str_add(&out, "+=");
	str_add(&out, expr_to_coffee(CHILD4,-1).data);
    ...
    str_add(&out, expr_to_coffee(CHILDN,indent+1).data);
    ...
    while(it){
      ...
      str_add(&out, expr_to_coffee(((expr_t*)(it->data)),-1).data );
    ...
    while(it){
      ...
      if (it==(expr->children.head)){
        str_add(&out,(char*)&expr_to_coffee(ex,indent+1).data[indent*2]);
      }else{
        str_add(&out,expr_to_coffee(ex,indent+1).data);
      }
      it = it->next;
    }
    ...
    while(it){
      ...
      str_add(&out, ((tok_t*)(((expr_t*)(((expr_t*)(it->data))->children.head->data))->term))->val.data);
      str_add(&out,"=");
      str_add(&out,zero_to_coffee(  (type_t*)(((expr_t*)(((expr_t*)(it->data))->children.head->next->data))->term) ).data);
      str_add(&out,"\n");
      it = it->next;
    }
    INDENT2(indent);
    ...
    if (typ->tag == TYP_STT){
      ...
      if (expr->children.len == 1){
        ...
        str_add(&out,s);
        str_add(&out,")['fill'](");
        str_add(&out,zero_to_coffee(typ->elem0).data);
        str_add(&out,"))");
      }else{
        str_add(&out,"[");
        list_node_t* it = expr->children.head->next;
    ...
    str_add(&out,expr_to_coffee(CHILD2,-1).data);
    str_add(&out,"]??");
    str_add(&out,zero_to_coffee(CHILD1->type->elem1).data);
    str_add(&out,")");
    ...
    str_add(&out,str_unquote(expr_to_coffee(CHILD1,-1)).data);
    indent=-1;
...
str_t tree_to_coffee(str_t modname, expr_t* tree, map_t* functable, map_t* stttable, map_t* included){
  ...
  INDENT2(1);
  str_addconst(&out,TEXT_std_coffee);
  INDENT2(1);
  ...
  while(it){
      ...
      if (ex1->key == EXPR_SET){
        ...
        if (ex2->key == EXPR_TERM){
          if (str_eq( &((tok_t*)(CHILD1->term))->val, ((tok_t*)(ex2->term))->val.data )){
            str_add(&out, ((tok_t*)(CHILD1->term))->val.data);
            str_add(&out,"=");
            str_add(&out,expr_to_coffee( (expr_t*)(ex1->children.head->next->data),-1).data);
            str_add(&out,"\n");
            it = it -> next -> next;
            continue;
          }   
    ...
    str_add(&out,expr_to_coffee(expr,1).data);
    it = it->next;
  }
  ...
  if (fun != NULL){
    if (!(fun->params.len)){
	  INDENT2(1);
      str_add(&out,"if (typeof process=='undefined') then main() else process.exit(main())\n");
    }else{
	  INDENT2(1);
      str_add(&out,"if (typeof process=='undefined') then main([]) else process.exit(main(process.argv.slice(1)))");
    }
  }
